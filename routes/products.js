...
const { productRules, idParamRule, validate } = require('../middleware/validators');
...
router.post('/', protect, upload.single('image'), productRules, validate, create);

// PUT /api/products/:id      — admin only, optional image replacement
router.put('/:id', protect, upload.single('image'), idParamRule, validate, update);
